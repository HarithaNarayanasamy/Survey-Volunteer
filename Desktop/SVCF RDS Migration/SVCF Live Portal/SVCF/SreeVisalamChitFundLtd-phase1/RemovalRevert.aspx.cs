...
{
    public partial class RemovalRevert : System.Web.UI.Page
    {
        #region Object
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (usrRole == "Administrator")
                {
                    ...
                    ChitGrp = balayer.GetDataTable("select distinct(Head_Id),GROUPNO from svcf.removal_approval as ta join svcf.groupmaster as gm on(ta.ChitNO=gm.Head_Id) where ta.BranchID=" + (Session["Branchid"]) + "");
        ...
        protected void ddlGroupNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Chit = balayer.GetDataTable("select mg.Head_Id,concat(mg.GrpMemberID,'|',mg.MemberName) as GrpMemberID from svcf.removal_approval as ta join svcf.membertogroupmaster as mg on(ta.GroupMemberID=mg.Head_Id) join membermaster as mm on(ta.OldMemberID=mm.MemberIDNew)  where ta.BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + "  and ta.ChitNO=" + ddlGroupNo.SelectedValue + "");
        ...
        protected void btnrevert_Click(object sender, EventArgs e)
        {
            ...
            {
                query = "select * from svcf.removal_approval as ta join membertogroupmaster as mg on(ta.NewMemberID=mg.MemberID) join membermaster as mm on(ta.OldMemberID=mm.MemberIDNew) where ta.BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and ta.ChitNO=mg.GroupID and ta.ChitNO=" + ddlGroupNo.SelectedValue + " and ta.GroupMemberID=" + ddlChit.SelectedValue + "";
                ...
                if (dtmember.Rows.Count > 0)
                {
                    ...
                    memberid = Convert.ToString(dtmember.Rows[0]["OldMemberID"]);
