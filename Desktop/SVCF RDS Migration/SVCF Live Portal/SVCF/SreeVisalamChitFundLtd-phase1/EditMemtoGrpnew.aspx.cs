...
{
    public partial class EditMemtoGrpnew : System.Web.UI.Page
    {
        #region Object
                ...
                {               
                    ...
                    for (int j = 0; j < dtHeads.Rows.Count; j++)
                    {
                        ...
                        if (!(string.IsNullOrEmpty(tokennumber)))
                        {
                            ...
                            //string Branchname = balayer.GetSingleValue(@"SELECT b1.B_Name FROM svcf.membermaster as m1 join svcf.branchdetails as b1 on (m1.BranchID=b1.Head_Id) join svcf.membertogroupmaster as mg1 on mg1.MemberID=m1.MemberIDNew where mg1.Head_Id=" +dtHeads.Rows[j]["NodeID"]+"");
                            if (string.IsNullOrEmpty(Branchname))
                            {
                              ...
                              //  drBind["B_Name"] = balayer.GetSingleValue("select B_Name from branchdetails where Head_Id=" + branchid);
                                drBind["B_Name"] = balayer.GetSingleValue(" SELECT b1.B_Name FROM svcf.membertogroupmaster as m1 join branchdetails as b1 on(m1.B_Id = b1.Head_Id) where m1.Head_id = " + dtHeads.Rows[j]["NodeID"] + "");
                            }
                            else
                            {
        ...
        public void gvDetails_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                //DropDownList ddlmembername = (DropDownList)e.Row.FindControl("ddlmembername");
                ...
                //dtChitGrp = balayer.GetDataTable(data);
                //ddlmembername.DataSource = dtChitGrp;
                //dr = dtChitGrp.NewRow();
                //dr[0] = "--select--";
                //dr[1] = "0";
                //dtChitGrp.Rows.InsertAt(dr, 0);
                ...
                //if (ddlmembername.Items.FindByText(hdnval.Value) != null)
                //    ddlmembername.Items.FindByText(hdnval.Value).Selected = true;
