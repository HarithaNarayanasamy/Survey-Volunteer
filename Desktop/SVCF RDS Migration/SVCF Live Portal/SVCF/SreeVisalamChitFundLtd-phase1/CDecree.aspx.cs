...
{
    public partial class CDecree : System.Web.UI.Page
    {
        ...
        ILog logger = log4net.LogManager.GetLogger(typeof(CDecree));
                        ...
                        if (lblHint.Text.ToLower() == "vconfirm")
                        {
                            ...
                            try
                            {
                                ...
                                for (int iTrans = 0; iTrans < GridGuardians.Rows.Count; iTrans++)
                                {
                                    ...
                                    TextBox txtCC = (TextBox)GridGuardians.Rows[iTrans].FindControl("txtCC");
                                    TextBox txtARC = (TextBox)GridGuardians.Rows[iTrans].FindControl("txtARC");
                                    ...
                                    long insertTransCourt = trn.insertorupdateTrn("insert into `svcf`.`transcourt` (DualTransactionKey,TransactionKey,Amount,ChoosenDate,HeadID,Narration,BranchID,VoucherType,CC,Number) values (" + DualTransactionKey + "," + iResult + "," + txtSubAmount.Text + ",'" + balayer.indiandateToMysqlDate(txtDate.Text) + "'," + ddlSubsHeads.SelectedValue.Split(',')[1].ToString().Trim() + ",'" + balayer.ReplaceJnk(txtDescription.Text) + "'," + Session["Branchid"] + ",'C','"+txtCC.Text+"','"+balayer.MySQLEscapeString(txtARC.Text)+"')");
                                }
                                for (int iTrans = 0; iTrans < GridGuardiansDebit.Rows.Count; iTrans++)
                                {
                                    DropDownList ddlSubsHeads = (DropDownList)GridGuardiansDebit.Rows[iTrans].FindControl("ddlHeadsDebit");
                                ...
                                trn.CommitTrn();
                                logger.Info("CDecree.aspx - btnYes_Click() - Completed: " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                            }
                            catch (Exception ex)
                            {
                                try
                                {
                                   trn.RollbackTrn();
                                   logger.Info("CDecree.aspx - btnYes_Click() - Error: " + ex.Message + ": " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                                }
                                catch
                                {
                                }
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            string ResExist = balayer.GetSingleValue("select count(*) from voucher where ChoosenDate>='2014/04/01' and BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and Series='" + txtSeries.Text.ToUpper().Trim() + "' and  Voucher_No=" + txtVoucherNo.Text.Trim() + " and Trans_Type='0' and Series='DECREE'");
