...
namespace SreeVisalamChitFundLtd_phase1
{
    public partial class EditAssignReceiptBook : System.Web.UI.Page
    {
        ...
        protected void gridBranch_RowValidating(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e)
        {
            ...
            var fromTxt = e.NewValues["receiptnofrom"];
            var toTxt = e.NewValues["receiptnoto"];
            var lastTxt = e.NewValues["alreadyusedReceipts"];

            if (Convert.ToInt32(lastTxt) < Convert.ToInt32(fromTxt) || Convert.ToInt32(lastTxt) > Convert.ToInt32(toTxt))
            {
                e.Errors[(GridViewDataColumn)grid.Columns[9]] = "Last Used Value Should be between from and to receipt numbers";

                if (e.Errors.Count > 0)
                {
                    e.RowError = e.Errors[grid.Columns[9]];
                }
            }
        }
