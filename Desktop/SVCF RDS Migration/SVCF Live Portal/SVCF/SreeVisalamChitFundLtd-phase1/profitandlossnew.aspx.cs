...
{
    public partial class profitandlossnew : System.Web.UI.Page
    {
        ...
        ILog logger = log4net.LogManager.GetLogger(typeof(profitandlossnew));
        ...
        {
            DataTable dtAllHeads = balayer.GetDataTable("SELECT concat(cast(  v1.`RootID` as char),',',cast(v1.`TreeID` as char)) as ID,v1.`TREE`,m1.`BranchID`,b1.BranchID,b2.Head_Id FROM `svcf`.`view_parent` as v1 left join `membertogroupmaster` as m1 on (v1.`TreeID` =m1.Head_Id) left join `bankdetails` as b1 on (v1.`TreeID` =b1.Head_Id) left join `branchdetails` as b2 on (v1.`TreeID` =b2.Head_Id) where v1.RootID in(11) and (m1.`BranchID` is null or m1.`BranchID`=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + ") and (b1.BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " or b1.BranchID is null) and (b2.Head_Id<>" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " or b2.Head_Id is null)");
            DataRow dr = dtAllHeads.NewRow();
            dr[0] = "--Select--";
            dr[1] = "--Select--";
            dtAllHeads.Rows.InsertAt(dr, 0);
                        ...
                        if (lblHint.Text.ToLower() == "vconfirm")
                        {
                            ...
                            try
                            {
                                ...
                                for (int iTrans = 0; iTrans < GridGuardians.Rows.Count; iTrans++)
                                {
                                    // trans_medium = "0";
                                    DropDownList ddlSubsHeads = (DropDownList)GridGuardians.Rows[iTrans].FindControl("ddlHeads");
                                    DropDownList ddlSubsHeadsDeb = (DropDownList)GridGuardiansDebit.Rows[iTrans].FindControl("ddlHeadsDebit");
                                    //By nithi
                                    //---------------
                                    int rootid = int.Parse(ddlSubsHeads.SelectedValue.ToString().Split(',')[0].ToString().Trim().Trim(',').Trim());
                                    if (rootid == 5)
                                    ...
                                    if (!string.IsNullOrEmpty(txtChk.Text) & txtChk.Visible)
                                    {
                                        ...
                                        string strBankInsertQuery = "insert into transbank(BranchID, TransactionKey, DualTransactionKey, T_Day, T_Month, T_Year, BankHeadID, Head_Id, MemberID, CustomersBankName, DateInCheque, ChequeDDNO, GivenAmount, TotalChequeDDAmount, IsBounced, Trans_Type) values (" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + "," + iResult + "," + DualTransactionKey + "," + dtChoosenDate.Day + "," + dtChoosenDate.Month + "," + dtChoosenDate.Year + "," + ddlSubsHeads.SelectedValue.Split(',')[1]+ ","+ddlSubsHeadsDeb.SelectedItem.Value.Split(',')[0]+",000,'','" + balayer.indiandateToMysqlDate(txtDate.Text) + "'," + txtChk.Text + "," + txtSubAmount.Text + "," + txtSubAmount.Text + ",0,0)";
                                        trn.insertorupdateTrn(strBankInsertQuery);
                                        long pResult = trn.insertorupdateTrn("insert into svcf.transprofitandloss (BranchID,DualTransactionKey,TransactionKey,HeadID,Narration,Amount,OppositeHeadId,ApplyedOn,Flag,aosanction,ChequeNo) values (" + Session["Branchid"] + "," + DualTransactionKey + "," + iResult + "," + ddlSubsHeadsDeb.SelectedItem.Value.Split(',')[1] + ",'" + balayer.MySQLEscapeString(txtDescription.Text) + "'," + txtSubAmount.Text + "," + ddlSubsHeads.SelectedItem.Value.Split(',')[1] + ",'" + balayer.indiandateToMysqlDate(txtDate.Text) + "',0," + txtVoucherNo.Text + "," + txtChk.Text + ")");
                                    }
                                    else
                                    {
                                        long pResult = trn.insertorupdateTrn("insert into svcf.transprofitandloss (BranchID,DualTransactionKey,TransactionKey,HeadID,Narration,Amount,OppositeHeadId,ApplyedOn,Flag,aosanction) values (" + Session["Branchid"] + "," + DualTransactionKey + "," + iResult + "," + ddlSubsHeadsDeb.SelectedItem.Value.Split(',')[1] + ",'" + balayer.MySQLEscapeString(txtDescription.Text) + "'," + txtSubAmount.Text + "," + ddlSubsHeads.SelectedItem.Value.Split(',')[1] + ",'" + balayer.indiandateToMysqlDate(txtDate.Text) + "',0," + txtVoucherNo.Text + ")");
                                    }
                                }
                                //trans_medium = "0";
                                for (int iTrans = 0; iTrans < GridGuardiansDebit.Rows.Count; iTrans++)
                                ...
                                trn.CommitTrn();
                                logger.Info("profitandlossnew.aspx - btnYes_click():  Completed: " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                            }
                            catch (Exception ex)
                            {
                                try
                                {
                                   trn.RollbackTrn();
                                   logger.Info("profitandlossnew.aspx - btnYes_click():  Error: " + ex.Message + ": " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                                }
                                catch
                                {
                                }
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            string ResExist = balayer.GetSingleValue("select count(*) from voucher where Series='" + txtSeries.Text.ToUpper().Trim() + "' and  Voucher_No=" + txtVoucherNo.Text.Trim() + " and Trans_Type='0'");
        ...
        {
            txtVoucherNo.Text = balayer.GetSingleValue("SELECT ifnull(max(cast(Voucher_No as unsigned)),0)+1 FROM svcf.voucher where Series='VOUCHER' and  ChoosenDate>='2014/04/01' and BranchID=" + Session["Branchid"]);
        }
