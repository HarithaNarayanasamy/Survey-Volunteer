        ...
        public void LoadGrpId(DropDownList ddl,int iType)
        {
            try
            {
                ...
                {                       
                    ...
                    case 2:
                        //16/07/2021->to block Tirunelveli -M chit, Karaikudi -M chit, Coimbatore -M chit
                        ...
                        qry = "select m.moneycollid,concat(m.moneycollname," + "' | '" + ",(SELECT Node FROM svcf.headstree where NodeID=m.BranchID))as moneycollname FROM moneycollector m where m.BranchID not in (159,1485,1113841);";
                        Tempdic = balayer.CmnList(qry);
                      ddl.DataValueField = "Key";
                      ddl.DataTextField = "Value";
                      ddl.DataSource = Tempdic;
