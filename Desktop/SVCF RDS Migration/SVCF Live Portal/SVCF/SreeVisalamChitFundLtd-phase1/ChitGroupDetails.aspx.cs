...
{
    public partial class ChitGroupDetails : System.Web.UI.Page
    {
        #region Object
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                cmbCommissionId.Items.Add("--select--");
                cmbCommissionId.Items.Add("5%");
                cmbCommissionId.Items.Add("6%");
                cmbCommissionId.Items.Add("7%");
                chitValue();
                if (!this.IsPostBack)
                {
        ...
        protected void btnaddGroup_click(object sender, EventArgs e)
        {
            ...
            try
            {
                txtgroup_no.Text = txtgroup_no.Text.Trim();
                ...
                //// string AucAMPM = Convert.ToString((string.Format("{0}", TimeSelector8.AmPm)));
                //string AucAMPM = "";
                //if (TimeSelector8.AmPm.ToString().Contains("AM"))
                //{
                //    AucAMPM = "AM";
                //}
                //else
                ...
                //    AucAMPM1 = "PM";
                //}
                //string AuctionEndTime = balayer.Get24HrTime(AucHour1, AucMin1, AucAMPM1, 0);
                string time = "00:00:00";   //09/09/2021
                ...
                DataTable selectedBranchTable = balayer.GetDataTable("select MemberID,AddressForCommunication from membermaster where BranchId=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and CustomerName like '%Sree Visalam%' ");
                ...
                // insert portion
                //09/09/2021
                //long insertCmd = trn.insertorupdateTrn("insert into groupmaster(BranchID,GROUPNO,Commission_ID,ChitValue,ChitPeriod,ChitCategory,ChitStartDate,NoofMembers,AuctionTime,AuctionEndTime,`Head_Id`) values(" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + ",'" + balayer.MySQLEscapeString(txtgroup_no.Text) + "','" + balayer.MySQLEscapeString(cmbCommissionId.SelectedItem.Value) + "','" + balayer.MySQLEscapeString(txtchit_value.SelectedItem.Value) + "','" + balayer.MySQLEscapeString(txtchit_period.Text) + "','" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(cmbchit_category.SelectedValue)) + "','null','"  + balayer.MySQLEscapeString(txtno_of_members.Text) + "','" + balayer.MySQLEscapeString(AuctionTime) + "','"+ balayer.MySQLEscapeString(AuctionEndTime) + "','"+iresult + "')");
                long insertCmd = trn.insertorupdateTrn("insert into groupmaster(BranchID,GROUPNO,Commission_ID,ChitValue,ChitPeriod,ChitCategory,ChitStartDate,NoofMembers,AuctionTime,AuctionEndTime,`Head_Id`) values(" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + ",'" + balayer.MySQLEscapeString(txtgroup_no.Text) + "','" + balayer.MySQLEscapeString(cmbCommissionId.SelectedItem.Value) + "','" + balayer.MySQLEscapeString(txtchit_value.SelectedItem.Value) + "','" + balayer.MySQLEscapeString(txtchit_period.Text) + "','" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(cmbchit_category.SelectedValue)) + "','null','" + balayer.MySQLEscapeString(txtno_of_members.Text) + "','" + time + "','" + time + "','" + iresult + "')");
                //
