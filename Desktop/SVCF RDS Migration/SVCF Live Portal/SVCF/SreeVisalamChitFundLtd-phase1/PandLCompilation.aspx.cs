...
namespace SreeVisalamChitFundLtd_phase1
{
    public partial class PandLCompilation : System.Web.UI.Page
    {
        ...
        public void LoadIncomeGrid()
        {
            ...
            //sivanesan created a new data table
            DataTable dtNodecolumns = new DataTable();

            dt.Columns.Add("SNo");
            dt.Columns.Add("BranchName");

            //string qry1 = "select NodeID,Node from headstree where Rootid=11 and ParentID=11\r\n";

            //string qry1 = "SELECT DISTINCT n1.NodeId, n1.Node FROM svcf.headstree AS n1 RIGHT JOIN svcf.voucher AS v1 ON v1.Head_ID = n1.NodeID WHERE n1.Rootid = 11 AND n1.ParentID = 11;";
            string qry1 = "SELECT * FROM svcf.headstree where Rootid=11;";

            dtNodecolumns = balayer.GetDataTable(qry1);

            foreach (DataRow row in dtNodecolumns.Rows)
            {
                string columnName = row["Node"].ToString();
            ...
            //dt.Columns.Add("Miscellaneous Income", typeof(decimal));
            ...
            try
            {
                ...
                foreach (DataColumn col in dt.Columns)
                {
                    if (col.ColumnName != "SNo" && col.ColumnName != "BranchName")
                    {
                        ...
                        // Check if the sum is non-zero before adding it to the DataRow
                        if (decTotal != 0)
                        {
                ...
                // Remove columns with zero sums outside of the loop
                foreach (DataColumn col in dt.Columns.Cast<DataColumn>().ToArray())
                {
                    if (col.ColumnName != "SNo" && col.ColumnName != "BranchName")
                    {
                        decTotal = Convert.ToDecimal(dt.Compute("Sum([" + col.ColumnName + "])", ""));
                        if (decTotal == 0)
                        {
                ...
                // Add the DataRow to the DataTable if it contains any non-zero sums
