...
namespace SreeVisalamChitFundLtd_phase1.UserControl
{
    public partial class FrmLedger : System.Web.UI.UserControl
    {
        ...
        //public void chitnumber()
        //{
        //    DataTable jj = balayer.GetDataTable("select uuid_from_bin(v1.DualTransactionKey) as dd,m1.GroupID as gid,v1.Voucher_No,v1.BranchID from svcf.voucher as v1 join svcf.membertogroupmaster as m1 on(v1.Head_Id=m1.Head_Id) where v1.Series='ADVICE' and v1.ChitGroupId=0");
        //    for (int i = 0; i < jj.Rows.Count; i++)
        //    {

        //        // string chitgroupid = balayer.GetSingleValue("select * from svcf.membertogroupmaster where Head_Id="+jj.Rows[i][""]);
        //        long insertCmd = trn.insertorupdateTrn("update  svcf.voucher set ChitGroupId=" + jj.Rows[i]["gid"] + " where uuid_from_bin(DualTransactionKey)='" + jj.Rows[i]["dd"] + "'");
        //    }
        //}
        public void LoadLedger()
        {
          // chitnumber();
            try
            {
                ...
                double tota=0;
                double balance = 0;
                double balanceminus = 0;
                double totalvalue = 0;
                double amt = 0;
                double previousvalue = 0;
                double currentvalue=0;
                Boolean amtvalue = false;
                //  string narr="";
                #endregion
                ...
                fillgrid.Columns.Add("Narration", typeof(string));
                fillgrid.Columns.Add("Noofinstallments", typeof(string));
                ...
                //qry = "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
                //    "and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //    "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //    "select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
                //    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
                //    " where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION  select  null as series, NULL as 'Date1' ,t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,NULL as PrizedAmount,a1.KasarAmount as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION " +
                //    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,a1.TotalCommission as  TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " order by Date";

                //Commented on 23/01/2021 - for bind AppReceiptno in the field of voucher_no

                //            qry = "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
                //"and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //"select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where   t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //"select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
                //"select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
                //" where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION  select  null as series, NULL as 'Date1' ,t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,NULL as PrizedAmount,a1.KasarAmount as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION " +
                //"select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,a1.TotalCommission as  TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " order by Date";
                ...
                //23/01/2021
                //            qry = "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,case when t2.AppReceiptno is not null then t2.AppReceiptno else t2.Voucher_No end as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
                //"and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //"select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where   t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
                //"select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
                //"select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
                ...
                //22/05/2021 - AppReceiptno for ADVICE entries
                /*           qry = "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.AppReceiptno ,case when t2.AppReceiptno <> null and t2.AppReceiptno='' then t2.AppReceiptno else t2.Voucher_No end as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
    "and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
    "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.AppReceiptno ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where   t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
    "select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.AppReceiptno,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
    " where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION  select  null as series, NULL as 'Date1' ,t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,NULL as PrizedAmount,a1.KasarAmount as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " UNION " +
    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,a1.TotalCommission as  TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " order by Date";
               */
                /*

                qry = "select t2.appreceiptno,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.series ,case when t2.AppReceiptno <> null and t2.AppReceiptno='' then t2.AppReceiptno else t2.Voucher_No end as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
    "and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
    "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.AppReceiptno ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where   t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
    "select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.AppReceiptno,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
    ...
    "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,a1.TotalCommission as  TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " order by Date";
    */
                //07/11/2021-bagya appreceiptno shown instead of series
                qry = "select t2.appreceiptno,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.series ,case when t2.AppReceiptno is null or t2.AppReceiptno='' then t2.Voucher_No else t2.AppReceiptno end as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,case when t2.Voucher_Type='D' then t2.Amount else 0.00 end as vamount " + " FROM svcf.voucher as t2 where ChitGroupID=" + DD_GP.SelectedValue + " and t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series<>'PAYMENT' and  t2.Series<>'FCP' and t2.Other_Trans_Type<>3 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')" +
        "and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
        "select t2.Series,t2.CurrDate as 'Date1', t2.ChoosenDate as 'Date',t2.Narration as narr,t2.AppReceiptno ,t2.Voucher_No as 'Narration', case when t2.Voucher_Type='C' then t2.Amount else 0.00 end  as Amount,t2.Head_ID as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount  FROM svcf.voucher as t2 where   t2.RootID=5 and t2.Other_Trans_Type<>5 and t2.Series='PAYMENT' and Trans_Type=1 and (t2.Voucher_Type='C' or t2.Voucher_Type = 'D')and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" + Session["Branchid"] + " UNION " +
        "select null as series, t1.CurrDate as 'Date1',t1.ChoosenDate as 'Date',NULL as narr,t1.AppReceiptno,t1.Narration as 'Narration',null as 'Amount',t1.Head_Id as headid,NUll as TotalCommission,NULL as PrizedAmount,null as kaser,t1.Amount as divdue,null as vamount FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "  UNION  " +
        "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,NULL as  TotalCommission,t1.PrizedAmount as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  " +
        ...
        "select null as series, NULL as 'Date1',t1.PaymentDate as 'Date',NULL as narr,NULL as AppReceiptno,NULL as  'Narration', NULL as Amount,NULL as headid,a1.TotalCommission as  TotalCommission,NULL as PrizedAmount,null as kaser,null as divdue,null as vamount from auctiondetails as a1 join trans_payment as t1 on (a1.PrizedMemberID =t1.TokenNumber)  where a1.PrizedMemberID=" + DD_GPMem.SelectedValue + " order by Date";
                ...
                string narration = balayer.GetSingleValue("select Narration FROM svcf.voucher as t2 where ChitGroupID=" +
                    "" + DD_GP.SelectedValue + " and Voucher_Type='D' and Series in('PAYMENT' , 'FCP') and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" +
                    "" + Session["Branchid"] + "");

                DataTable dt2 = balayer.GetDataTable("select Amount,Choosendate FROM svcf.voucher as t2 where ChitGroupID=" +
                   "" + DD_GP.SelectedValue + " and Voucher_Type='D' and Series in('PAYMENT' , 'FCP') and t2.Head_ID=" + DD_GPMem.SelectedValue + " and `t2`.`BranchID` =" +
                   "" + Session["Branchid"] + "");
                ...
                tempDueAmount = 0;
                tokenpaidamnt = 0;
                amtvalue = false; string previousDrawNo = ""; double previousSubscription = 0;
                for (int rw = 0; rw <= dt1.Rows.Count - 1; rw++)
                {
                    if (Convert.ToString(dt1.Rows[rw]["Date"]) == "2018-04-30")
                    {
                    ...
                    try
                    {
                        qry = "select DrawNO,CurrentDueAmount,PrizedMemberID from auctiondetails where GroupID=" + DD_GP.SelectedValue + "";
                        findrdt = balayer.GetDataTable(qry);
                        if (Convert.ToString(dt1.Rows[rw]["Narration"]) != "")
                        {
                            if (Convert.ToString(dt1.Rows[rw]["Amount"]) != "")
                            {
                                tokenpaidamnt = tokenpaidamnt + Convert.ToDouble(dt1.Rows[rw]["Amount"]);
                            }
                            //13/05/2021
                            //if (Convert.ToString(dt1.Rows[rw]["Series"]) == "BCAPP" || Convert.ToString(dt1.Rows[rw]["Series"]) == "CPAPP" || Convert.ToString(dt1.Rows[rw]["Series"]) == "CPWEB")
                            //{
                            //    string nar = Convert.ToString(dt1.Rows[rw]["narr"]);
                            //    string[] narrat = nar.Split(':');
                            //    drow["Noofinstallments"] = narrat[4];
                            //}
                            ////22/05/2021 - Bala
                            //else if (!string.IsNullOrEmpty(dt1.Rows[rw]["AppReceiptno"].ToString()))
                            //{
                            //    string nar = Convert.ToString(dt1.Rows[rw]["narr"]);
                            //    string[] narrat = nar.Split(':');
                            //    drow["Noofinstallments"] = narrat[6];
                            //}

                            //13 / 05 / 2021 - Commented & altered for Installment no
                            //else
                            //{
                                decimal recentPaid = 0;
                                if (!string.IsNullOrEmpty(dt1.Rows[rw]["Amount"].ToString()))
                                    {
                                     recentPaid = Convert.ToDecimal(dt1.Rows[rw]["Amount"]);
                                }
                                ...
                                //    else
                                //    {
                                //        //double gg =Convert.ToDouble( findrdt.Rows[lt]["DrawNO"].ToString());
                                ...
                                //            {
                                //                //drow["Noofinstallments"] = findrdt.Rows[lt]["DrawNO"];
                                //                if (previousDrawNo != Convert.ToString(findrdt.Rows[lt]["DrawNO"]))
                                //                {
                                //                    drow["Noofinstallments"] = findrdt.Rows[lt]["DrawNO"];
                                //                    previousDrawNo = Convert.ToString(findrdt.Rows[lt]["DrawNO"]);
                                //                }
                                //                else
                                //                {
                                ...
                                decimal TotalPaidAmount = Convert.ToDecimal(tokenpaidamnt);
                                ...
                                if (TotalPaidAmount == 0.00M)
                                {
                                    FromNarration = "1";
                                    FromDraw = 1;
                                    TotalPaidAmount = Convert.ToDecimal( tokenpaidamnt);
                                    DataTable dtAuction = balayer.GetDataTable("SELECT DrawNO,CurrentDueAmount FROM `svcf`.`auctiondetails` where GroupID=" + DD_GP.SelectedValue + " and CurrentDueAmount<>'0.00' order by DrawNO");
                                    for (int iAuc = 0; iAuc < dtAuction.Rows.Count; iAuc++)
                                    {
                                        ...
                                        decimal tempDueAmount1 = TotalPaidAmount;
                                        if (tempDueAmount1 == 0.00M)
                                        {
                                            ToNarration = (iAuc + 1).ToString();
                                            ToDraw = iAuc + 1;
                                            break;
                                        }
                                        else if (tempDueAmount1 < 0.00M)
                                        {
                                            ToDraw = iAuc + 1;
                                    ...
                                    drow["Noofinstallments"] = FromNarration;
                                ...
                                else
                                {
                                    ...
                                    for (int iAuc = 0; iAuc < dtAuction.Rows.Count; iAuc++)
                                    {
                                        ...
                                        TotalPaidAmount = TotalPaidAmount - currentDueAmount;
                                        //10/06/2021
                                        if(TotalPaidAmount==recentPaid)
                                        {
                                            FromDraw = iAuc+2;
                                        }
                                        else if(TotalPaidAmount<recentPaid && FromDraw==0)
                                        {
                                            FromDraw = iAuc + 1;
                                        }
                                        decimal tempDueAmount1 = TotalPaidAmount;
                                        if (tempDueAmount1 == 0.00M)
                                        {
                                            if(FromDraw!=0)
                                            {
                                                FromNarration = FromDraw.ToString() + "F";
                                            }
                                            //FromNarration = (iAuc + 1).ToString()+"F";
                                            //FromDraw = iAuc + 1;
                                    ...
                                    if (ToNarration != "")
                                    {
                                        if (FromDraw != ToDraw && FromDraw!=0)
                                        {
                                            drow["Noofinstallments"] = FromNarration + " To " + ToNarration;
                                        }
                                        else
                                        {
                                            drow["Noofinstallments"] = ToNarration;
                                    ...
                                    if (FromNarration == "")
                                        drow["Noofinstallments"] = dtAuction.Rows.Count + " + Excess";
                                    else
                                        drow["Noofinstallments"] = FromNarration;
                                }
                                }
                                ...
                                if (Convert.ToString(dt1.Rows[rw]["Series"]) == "VOUCHER" || Convert.ToString(dt1.Rows[rw]["Series"]) == "PAYMENT")
                                {
                                    drow["Narration"] = dt1.Rows[rw]["narr"];
                                }
                                else
                                {
                                ...
                                if (Convert.ToString(drow["Noofinstallments"]) != "")
                                {
                                ...
                                if (Convert.ToString(dt1.Rows[rw]["Narration"]) == "")
                                {
                                    ...
                                    if (Convert.ToString(dt1.Rows[rw]["PrizedAmount"]) != "")
                                    {
                                        drow["Narration"] = narration;
                                        drow["Noofinstallments"] = "";
                                        drow["ShareAmount"] = "";
                                        drow["Subscription"] = "";
                                        drow["Total"] = "";
                                        drow["PrizedAmount"] = dt1.Rows[rw]["PrizedAmount"];
                                        drow["KasarAmount"] = "";
                                        drow["Balance"] = "";
                                    }
                                    if (Convert.ToString(dt1.Rows[rw]["kaser"]) != "")
                                    {
                                        drow["Narration"] = "Dividend";
                                        drow["Noofinstallments"] = "";
                                        drow["ShareAmount"] = "";
                                        drow["Subscription"] = "";
                                        drow["Total"] = "";
                                        drow["PrizedAmount"] = dt1.Rows[rw]["kaser"];

                                        drow["KasarAmount"] = "";
                                        drow["Balance"] = "";
                                    }
                                    else if (Convert.ToString(dt1.Rows[rw]["TotalCommission"]) != "")
                                    {
                                        drow["Narration"] = "Commission";
                                        drow["Noofinstallments"] = "";
                                        drow["ShareAmount"] = "";
                                        drow["Subscription"] = "";
                                        drow["Total"] = "";
                                        drow["PrizedAmount"] = dt1.Rows[rw]["TotalCommission"];
                                        drow["KasarAmount"] = "";
                                        drow["Balance"] = "";
                                    }
                                ...
                                if (Convert.ToString(dt1.Rows[rw]["Amount"]) == "" || Convert.ToString(dt1.Rows[rw]["Amount"]) == "0.00")
                                {
                                    if (Convert.ToString(dt1.Rows[rw]["divdue"]) != "")
                                    {
                                        drow["Narration"] = dt1.Rows[rw]["Narration"];
                                        drow["Noofinstallments"] = "";
                                        drow["ShareAmount"] = "";
                                        ...
                                        drow["Total"] = "";
                                        drow["PrizedAmount"] = "";
                                        drow["KasarAmount"] = dt1.Rows[rw]["divdue"];
                                        balance = Convert.ToDouble(dt1.Rows[rw]["divdue"]);
                                        drow["Balance"] = "";
                                    }
                                    if (Convert.ToString(dt1.Rows[rw]["vamount"]) != "")
                                    {
                                        drow["Narration"] = dt1.Rows[rw]["narr"];
                                        //drow["Narration"] = "";
                                        drow["Noofinstallments"] = "";
                                        ...
                                        drow["Subscription"] = "";
                                        drow["Total"] = "";
                                        drow["PrizedAmount"] = dt1.Rows[rw]["vamount"];
                                        //drow["PrizedAmount"] = "";
                                        amtback = Convert.ToDouble(dt1.Rows[rw]["vamount"]);
                                ...
                                if (rw == 0)
                                {
                                    ...
                                    if (dt1.Rows[rw]["Amount"].ToString() == "")
                                    {
                                        existinginterest = 0;
                                    }
                                    else
                                ...
                                else if (Convert.ToString(dt1.Rows[rw]["Narration"]) != "" && Convert.ToString(dt1.Rows[rw]["Amount"]) != "")
                                {
                                    if (Convert.ToString(dt1.Rows[rw]["vamount"]) == "0.00" || Convert.ToString(dt1.Rows[rw]["vamount"]) == "")
                                    {
                                        existinginterest = existinginterest + Convert.ToDouble(dt1.Rows[rw]["Amount"]);
                                        exi = exi + Convert.ToDouble(dt1.Rows[rw]["Amount"]);
                                ...
                                {
                                    ///05/10/2018
                                    if (dt1.Rows[rw]["Date"].ToString() == dt2.Rows[0]["ChoosenDate"].ToString())
                                    {
                                        amtvalue = true;
                                    }
                                }
                                ...
                                if (amtvalue == true)
                                {
                                    if (Convert.ToString(drow["Total"]) != "" || Convert.ToString(drow["Narration"]) != "" || Convert.ToString(drow["Noofinstallments"]) != "" || Convert.ToString(drow["Subscription"]) != "" || Convert.ToString(drow["KasarAmount"]) != "" || Convert.ToString(drow["ShareAmount"]) != "" || Convert.ToString(drow["PrizedAmount"]) != "" || Convert.ToString(drow["Balance"]) != "")
                                    {
                                        if (Convert.ToString(dt1.Rows[rw]["Amount"]) != "0.00")
                                        {

                                            totalvalue = existinginterest;
                                            amt = Convert.ToDouble(dt2.Rows[0]["Amount"]);
                                            if (totalvalue > amt)
                                            {
                                                drow["Balance"] = Convert.ToDouble(totalvalue - amt);
                                            }
                                            else
                                            {
                                                drow["Balance"] = Convert.ToDouble(amt - totalvalue);
                                            }
                                        }


                                        else if (Convert.ToString(dt1.Rows[rw]["Amount"]) == "0.00" && Convert.ToString(dt1.Rows[rw]["vamount"]) == "0.00" || Convert.ToString(dt1.Rows[rw]["vamount"]) == "")
                                        {

                                            total = existinginterest;
                                            if (total > amtback)
                                            {
                                                tota = total - amtback;
                                                //drow["Total"] = tota;
                                            ...
                                            if (total > amt)
                                            {
                                                //drow["Balance"] = Convert.ToDouble(tota - amt);
                                                drow["Balance"] = Convert.ToDouble(total - amt);
                                            }
                                            else
                                            {
                                                // drow["Balance"] = Convert.ToDouble(amt - tota);
                                                drow["Balance"] = Convert.ToDouble(amt - total);
                                            }

                                            //existinginterest = tota;
                                            //tokenpaidamnt = tota;
                                            existinginterest = total;
                                            tokenpaidamnt = total;
                                        }
                                        else if (Convert.ToString(dt1.Rows[rw]["vamount"]) != "0.00")
                                        {
                                ...
                                else
                                {
                                    ...
                                    if (Convert.ToString(dt1.Rows[rw]["Amount"]) == "0.00" && Convert.ToString(dt1.Rows[rw]["vamount"]) == "0.00" || Convert.ToString(dt1.Rows[rw]["vamount"]) == "")
                                    {
                                        if (Convert.ToString(drow["Total"]) != "" || Convert.ToString(drow["Narration"]) != "" || Convert.ToString(drow["Noofinstallments"]) != "" || Convert.ToString(drow["Subscription"]) != "" || Convert.ToString(drow["KasarAmount"]) != "" || Convert.ToString(drow["ShareAmount"]) != "" || Convert.ToString(drow["PrizedAmount"]) != "" || Convert.ToString(drow["Balance"]) != "")
                                        {
                                            //drow["Total"] = tota;
                ...
                //       "and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + "";
                //   DataTable dt2 = new DataTable();
                //  dt2 = balayer.GetDataTable(qry);
                if (fillgrid.Rows.Count > 0)
                {
                    //subscription = Convert.ToString(fillgrid.Compute("sum(Subscription)", ""));
                    //divdueamount = Convert.ToString(fillgrid.Compute("sum(KasarAmount)", ""));
                    //DataRow dr2 = fillgrid.NewRow();


                    //dr2["Subscription"] = subscription;
                    //dr2["KasarAmount"] = divdueamount;
                    //fillgrid.Rows.Add(dr2.ItemArray);
                    //if (fillgrid.Rows.Count > 0)
                    //{
                    ...
                    captuion = "Sree Visalam Chit Fund Limited.,<br/>(Regd. Office: Tirunelveli-6. Central Office: Chennai-2)<br/>Ledger maintained under Section 23 of the Chit Funds Act 1982 and Rule 25 of the Tamilnadu Chit Funds Rules 1984<br/>Section : 1 Receipts and Payments in respect of subscribers<br/>Office where the Chit Agreement of the Chit is Registered:\n" + chitdet.Rows[0]["PSODROffice"].ToString() + "<br/>Registration Number of the Chit Agreement:" + chitdet.Rows[0]["ChitAgreementNo"].ToString() + "\n<br/>Branch Name : " + branchname + "; \t Group Name: " + DD_GP.SelectedItem.Text + " <br/> Member Name : " + DD_GPMem.SelectedItem.Text + "; \t Chit Amount : " + chitdet.Rows[0]["ChitValue"].ToString() + ";";
    ...
    //    }
    //    protected void GD_Ledger_RowCreated(object sender, GridViewRowEventArgs e)
    //    {
    ...
    //        (6,7, DataControlRowType.Header, DataControlRowState.Insert);
    ...
    //            AddMergedCells(objgridviewrow, objtablecell, 7,
    //            "Amount Paid By Subscriber", System.Drawing.Color.LightGreen.Name);
    ...
    //    }
        public void LoadLedgerold()
        {
            try
            {
                ...
                //Commented on 23/01/2021 - to bind AppReceiptno in the field of Voucher_No
                ...
                qry = "select date_format(t1.ChoosenDate,'%d-%m-%Y') as 'Date', case when t1.AppReceiptno is not null then t1.AppReceiptno else t1.Voucher_No end as 'Narration', case when t1.Voucher_Type='C' then t1.Amount else 0.00 end  as Amount,t1.Head_ID" +
                     " FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and t1.RootID=5 and t1.Other_Trans_Type<>5 and t1.Voucher_Type='C' " +
                     "and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + " order  by t1.ChoosenDate asc";
                DataTable dt1 = balayer.GetDataTable(qry);
        ...
        protected void imgpdf_Click(object sender, ImageClickEventArgs e)
        {
            ...
            string query = "select Node from svcf.headstree where ParentID=1 and NodeID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + "";
            string branchname = balayer.GetSingleValue(query);
            captuion = "Sree Visalam Chit Fund Limited.,<br/>(Regd. Office: Tirunelveli-6. Central Office: Chennai-2)<br/>Ledger maintained under Section 23 of the Chit Funds Act 1982 and Rule 25 of the Tamilnadu Chit Funds Rules 1984<br/>Section : 1 Receipts and Payments in respect of subscribers<br/>Office where the Chit Agreement of the Chit is Registered:\n" + chitdet.Rows[0]["PSODROffice"].ToString()+ "<br/>Registration Number of the Chit Agreement" + chitdet.Rows[0]["ChitAgreementNo"].ToString() + "<br/>Branch Name : " + branchname + "; \t Group Name: " + DD_GP.SelectedItem.Text + " <br/> Member Name : " + DD_GPMem.SelectedItem.Text + "; \t Chit Amount : " + chitdet.Rows[0]["ChitValue"].ToString() + ";";
            ...
            phrase.Add(new Chunk("Form No: XIV\n "));
            phrase.Add(new Chunk("Sree Visalam Chit Fund Limited.,\n ", FontFactory.GetFont("Arial", 15, 3, iTextSharp.text.Color.BLACK)));
            phrase.Add(new Chunk("(Regd. Office: Tirunelveli-6. Central Office: Chennai-2)\n", FontFactory.GetFont("Arial", 12, 3, iTextSharp.text.Color.BLACK)));
            phrase.Add(new Chunk("Ledger maintained under Section 23 of the Chit Funds Act 1982 and Rule 25 of the Tamilnadu Chit Funds Rules 1984 \n", FontFactory.GetFont("Arial", 12, 3, iTextSharp.text.Color.BLACK)));
            ...
            phrase.Add(new Chunk("Office where the Chit Agreement of the Chit is Registered:  "    + chitdet.Rows[0]["PSODROffice"].ToString()+"\n"));
            phrase.Add(new Chunk("\t\t\t\t\t\t\tRegistration Number of the Chit Agreement:  " + chitdet.Rows[0]["ChitAgreementNo"].ToString() + "\n"));
            ...
            for (int i = 0; i < GD_Ledger.Rows.Count; i++)
            {
                ...
                if (GD_Ledger.Rows[i].RowType == DataControlRowType.DataRow)
                {
                    ...
                    //9th colum
                    Label lblbalance = (Label)GD_Ledger.Rows[i].FindControl("lblbalance");
                    string cellText9 = Server.HtmlDecode(lblbalance.Text);
            ...
            iTextSharp.text.Document pdfDoc = new iTextSharp.text.Document(PageSize.A3.Rotate(), 0, 0, 5, 0);
        ...
        public void LoadGD_MemberList()
        {
            try
            {
                ...
                //Commented on 23/01/2021 - to bind AppReceiptno instead of Voucher_No
                ...
                //        "and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + " order  by t1.ChoosenDate";
                qry = "select case when t1.AppReceiptno is not null then t1.AppReceiptno else t1.Voucher_No end as 'Narration',t1.Amount as 'Amount' FROM svcf.voucher as t1 where ChitGroupID=" + DD_GP.SelectedValue + " and Series<>'A' and Trans_Type=1 and Other_Trans_Type=5 " +
                        "and t1.Head_ID=" + DD_GPMem.SelectedValue + " and `t1`.`BranchID` =" + Session["Branchid"] + " order  by t1.ChoosenDate";
                DataTable dt2 = new DataTable();
                dt2 = balayer.GetDataTable(qry);
