...
{
    public partial class DDecree : System.Web.UI.Page
    {
        ...
        ILog logger = log4net.LogManager.GetLogger(typeof(DDecree));
        ...
        private void SetGridTabIndex(bool isCredit)
        {
            ...
            else
            {
                ...
                foreach (GridViewRow dr in theGridView.Rows)
                {
                    ...
                    TextBox txtCC = (TextBox)dr.FindControl("txtCC");
                    TextBox txtARC = (TextBox)dr.FindControl("txtARC");
                    ...
                    ddlHeads.TabIndex = ++currentTabIndex;
                    txtAmount.TabIndex = ++currentTabIndex;
                    txtCC.TabIndex = ++currentTabIndex;
                    txtARC.TabIndex = ++currentTabIndex;
                    txtDescription.TabIndex = ++currentTabIndex;
                }
        ...
        {
            string query = "SELECT concat(cast(  v1.`RootID` as char),',',cast(v1.`TreeID` as char)) as ID,v1.`TREE` FROM `svcf`.`view_parent` as v1 " +
                            "where v1.`RootID`=7 and v1.BranchID=" + Session["Branchid"] + " union " +
                            "SELECT concat(cast(  v1.`RootID` as char),',',cast(v1.`TreeID` as char)) as ID,v1.`TREE` FROM `svcf`.`view_parent` as v1 where v1.`RootID`=1;";
            //DataTable dtAllHeads = balayer.GetDataTable("SELECT concat(cast(  v1.`RootID` as char),',',cast(v1.`TreeID` as char)) as ID,v1.`TREE` FROM `svcf`.`view_parent` as v1 where v1.`RootID`=7 and v1.BranchID=" + Session["Branchid"]);
            DataTable dtAllHeads = balayer.GetDataTable(query);
        ...
        protected void btnAdd_GridGuardiansDebit_RowCommand_click(object sender, ImageClickEventArgs e)
        {
            ...
            DataColumn dcCC = new DataColumn("CC", typeof(string));
            DataColumn dcARC = new DataColumn("ARC", typeof(string));
            ...
            dtDeb.Columns.Add(dcCC);
            dtDeb.Columns.Add(dcARC);
            foreach (GridViewRow dr in GridGuardiansDebit.Rows)
            {
                ...
                TextBox txtCC = (TextBox)dr.FindControl("txtCC");
                dtDbrow["CC"] = txtCC.Text;
                TextBox txtARC = (TextBox)dr.FindControl("txtARC");
                dtDbrow["ARC"] = txtARC.Text;
                TextBox txtDescription = (TextBox)dr.FindControl("txtDescriptionDebit");
        ...
        protected void btnRemove_GridGuardiansDebit_RowCommand_click(object sender, ImageClickEventArgs e)
        {
            ...
            dt1.Columns.Add(dcCC);
            dt1.Columns.Add(dcARC);
                        ...
                        if (lblHint.Text.ToLower() == "vconfirm")
                        {
                            ...
                            try
                            {
                                ...
                                for (int iTrans = 0; iTrans < GridGuardiansDebit.Rows.Count; iTrans++)
                                {
                                    ...
                                    TextBox txtCC = (TextBox)GridGuardiansDebit.Rows[iTrans].FindControl("txtCC");
                                    TextBox txtARC = (TextBox)GridGuardiansDebit.Rows[iTrans].FindControl("txtARC");
                                    ...
                                    long insertTransCourt = trn.insertorupdateTrn("insert into `svcf`.`transcourt` (DualTransactionKey,TransactionKey,Amount,ChoosenDate,HeadID,Narration,BranchID,VoucherType,CC,Number) values (" + DualTransactionKey + "," + iResult + "," + txtSubAmount.Text + ",'" + balayer.indiandateToMysqlDate(txtDate.Text) + "'," + ddlSubsHeads.SelectedValue.Split(',')[1].ToString().Trim() + ",'" + balayer.ReplaceJnk(txtDescription.Text) + "'," + Session["Branchid"] + ",'D','"+txtCC.Text+"','"+balayer.MySQLEscapeString( txtARC.Text)+"')");

                                }
                                trn.CommitTrn();
                                logger.Info("DDecree.aspx - btnYes_Click():  Completed: " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                            }
                            catch (Exception ex)
                            {
                                try
                                {
                                   trn.RollbackTrn();
                                   logger.Info("DDecree.aspx - btnYes_Click():  Error: " + ex.Message + ": " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            string ResExist = balayer.GetSingleValue("select count(*) from voucher where ChoosenDate>='" + DateTime.Now.Year + "/04/01' and BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and Voucher_No=" + txtVoucherNo.Text.Trim() + " and Trans_Type='0' and Series='DECREE'");
        ...
        protected void GetSeriesAndVoucherNo()
        {
            ...
            txtVoucherNo.Text = balayer.GetSingleValue("SELECT ifnull(max(cast(Voucher_No as unsigned)),0)+1 FROM svcf.voucher where ChoosenDate>='2014/04/01' and  Series='DECREE' and BranchID=" + Session["Branchid"]);
        }
        ...
        protected void GridGuardians_SelectedIndexChanged(object sender, EventArgs e)
        {
