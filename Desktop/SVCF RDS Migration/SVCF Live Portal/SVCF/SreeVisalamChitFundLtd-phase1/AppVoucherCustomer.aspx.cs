...
{
    public partial class AppVoucherCustomer : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else if (RadView.Checked == true)
                {
                    ...
                    approvalview.LoadWithDate(ddlSeries.SelectedItem.Text, txtFrom.Text, txtTo.Text);
                }
            }
        }
        ...
        //{
        //    var qry = "select moneycollid, moneycollname from moneycollector where BranchID = " + ddlBranch.SelectedItem.Value;
        //    tempDic.Clear();
        //    tempDic = balayer.CmnList(qry);
        //    ddlMoneyCollector.DataValueField = "Key";
        //    ddlMoneyCollector.DataTextField = "Value";
        //    ddlMoneyCollector.DataSource = tempDic;
        ...
        //        ddlSeries.Items.Add(new System.Web.UI.WebControls.ListItem("CPAPP", "Customer App"));
        //        ddlSeries.Items.Add(new System.Web.UI.WebControls.ListItem("CPWEB", "Website"));
        //}


        public DataTable getReceipts()
        {
            string series = ddlSeries.SelectedItem.Value;
            if (series == "--Select--")
            {
                DataTable dtReceipts = balayer.GetDataTable("SELECT mv.DualTransactionKey,mv.BranchID,b1.B_Name as BranchName,mv.Head_Id,mv.AppReceiptno,convert_tz( mv.CurrDate,'+00:00','+05:30') as CurrDate,mv.Amount,mv.Series,mv.M_Id,mv.ChitGroupId,mv.MoneyCollId,mv.TransactionKey,mv.ChoosenDate,mv.Interest,mv.Type,mg.GrpMemberID AS ChitNo FROM mobileappvoucher AS mv LEFT JOIN membertogroupmaster AS mg ON (mv.Head_Id = mg.Head_Id) left join branchdetails as b1 on(mv.BranchID=b1.Head_Id) WHERE mv.Voucher_Type = 'C' and (mv.Series='CPAPP' or mv.Series='CPWEB') and mv.IsAccepted=0 and mv.BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and mv.ChoosenDate between '" + balayer.changedateformat(Convert.ToDateTime(txtFrom.Text), 2) + "' and '" + balayer.changedateformat(Convert.ToDateTime(txtTo.Text), 2) + "';");
                return dtReceipts;
            }
            else
            {
                DataTable dtReceipts = balayer.GetDataTable("SELECT mv.DualTransactionKey,mv.BranchID,b1.B_Name as BranchName,mv.Head_Id,mv.AppReceiptno,convert_tz( mv.CurrDate,'+00:00','+05:30') as CurrDate,mv.Amount,mv.Series,mv.M_Id,mv.ChitGroupId,mv.MoneyCollId,mv.TransactionKey,mv.ChoosenDate,mv.Interest,mv.Type,mg.GrpMemberID AS ChitNo FROM mobileappvoucher AS mv LEFT JOIN membertogroupmaster AS mg ON (mv.Head_Id = mg.Head_Id) left join branchdetails as b1 on(mv.BranchID=b1.Head_Id) WHERE mv.Voucher_Type = 'C' and mv.Series='" + series + "' and mv.IsAccepted=0 and mv.BranchID=" + balayer.ToobjectstrEvenNull(Session["Branchid"]) + " and mv.ChoosenDate between '" + balayer.changedateformat(Convert.ToDateTime(txtFrom.Text), 2) + "' and '" + balayer.changedateformat(Convert.ToDateTime(txtTo.Text), 2) + "';");
                return dtReceipts;
            }
            //DataTable dtReceipts = balayer.GetDataTable("SELECT mv.DualTransactionKey,mv.BranchID,b1.B_Name as BranchName,mv.Head_Id,mv.AppReceiptno,convert_tz( mv.CurrDate,'+00:00','+05:30') as CurrDate,mv.Amount,mv.Series,mv.M_Id,mv.ChitGroupId,mv.MoneyCollId,mv.TransactionKey,mv.ChoosenDate,mv.Interest,mv.Type,mg.GrpMemberID AS ChitNo FROM mobileappvoucher AS mv LEFT JOIN membertogroupmaster AS mg ON (mv.Head_Id = mg.Head_Id) left join branchdetails as b1 on(mv.BranchID=b1.Head_Id) WHERE mv.Voucher_Type = 'C' and mv.Series='" + series + "' and mv.IsAccepted=0 and mv.BranchID=" + ddlBranch.SelectedItem.Value+ " and mv.MoneyCollId="+ddlMoneyCollector.SelectedItem.Value+" and mv.Series='"+ddlSeries.SelectedItem+"' and mv.ChoosenDate between '" + balayer.changedateformat(Convert.ToDateTime(txtFrom.Text), 2) + "' and '" + balayer.changedateformat(Convert.ToDateTime(txtTo.Text), 2) + "';");
            //return dtReceipts;
        }
        ...
        protected void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //voucher number
                //string vouQry = "select ifnull(max(cast(voucher_no as unsigned)),0)+1 from voucher where series='" + series + "' and Trans_Type=1 ;";
                var vouQry = balayer.GetSingleValue("SELECT ifnull(max(cast(Voucher_No as unsigned)),0)+1 FROM svcf.voucher where ChoosenDate>='2020/04/01' and  Series='" + series + "' and Trans_Type='1' and BranchID=161");
                int voucherNo = int.Parse(vouQry);
                ...
                //18/08/2021
                decimal totalTransBank = 0;
                if (dtVou.Rows.Count > 1)
                {
                    totalTransBank = Convert.ToDecimal(dtVou.Rows[0]["Amount"]) + Convert.ToDecimal(dtVou.Rows[1]["Amount"]);
                }
                else
                {
                    totalTransBank = Convert.ToDecimal(dtVou.Rows[0]["Amount"]);
                }
                //
                ...
                for (int i = 0; i < dtVou.Rows.Count; i++)
                {
                    ...
                    if (type == "Card")
                    {
                        ...
                        try
                        {
                            ...
                            "`Amount`,`Series`,`ReceievedBy`,`Trans_Type`,`T_Day`,`T_Month`,`T_Year`,`MemberID`,`Trans_Medium`,`RootID`,`ChitGroupId`,`Other_Trans_Type`,`AppReceiptno`,`IsDeleted`,`IsAccepted`,`M_Id`,`LoginIP`) VALUES ('" + DualTransactionKey + "'," +
                            balayer.TostrEvenNull(Convert.ToString(Session["Branchid"])) + ",'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'," + voucherNo + ",'D', '3','" + choosenDate.ToString("yyyy-MM-dd") + "','" + "Recd From:- - :-:" + appReceiptno + "-" + membername + ":" + TokenNo + " :For Inst No:" + cashNarration +
                            "'," + Convert.ToString(dtVou.Rows[i]["Amount"]) + ",'" + series + "','" + Convert.ToString(dtVou.Rows[i]["ReceievedBy"]) + "',1," + choosenDate.Day + "," + choosenDate.Month + "," +
                            ...
                            //17/08/2021 - Transbank table insertion

                            string qryBank = "insert into transbank(BranchID, TransactionKey, DualTransactionKey, T_Day, T_Month, T_Year, BankHeadID, Head_Id, MemberID, CustomersBankName, DateInCheque, ChequeDDNO, GivenAmount, TotalChequeDDAmount, IsBounced, Trans_Type) " +
                            "values(" + balayer.TostrEvenNull(Convert.ToString(Session["Branchid"])) + "," + insert1 + "," + DualTransactionKey + "," + choosenDate.Day + "," + choosenDate.Month + "," + choosenDate.Year + "," + 11111 /*BankHeadID(Id of name of the bank)*/ + "," + dtVou.Rows[i]["Head_Id"] + "," + memberId + ",'Online Transfer','" +
                            choosenDate.ToString("yyyy-MM-dd") + "'," + 123584/*BankReferenceNumber*/+ "," + Convert.ToString(dtVou.Rows[i]["Amount"]) + "," + totalTransBank + ",0,1)";

                            trn.insertorupdateTrn(qryBank);
                            //}
                        }
                        catch (Exception ex)
                        {
                    ...
                    else if (type == "DefaultInterest")
                    {
                        ...
                        try
                        {
                            ...
                            string qry2 = "INSERT INTO `svcf`.`voucher` (`DualTransactionKey`,`BranchID`,`CurrDate`,`Voucher_No`,`Voucher_Type`,`Head_Id`,`ChoosenDate`,`Narration`," +
                            "`Amount`,`Series`,`ReceievedBy`,`Trans_Type`,`T_Day`,`T_Month`,`T_Year`,`MemberID`,`Trans_Medium`,`RootID`,`ChitGroupId`,`Other_Trans_Type`,`AppReceiptno`,`IsDeleted`,`IsAccepted`,`M_Id`,`LoginIP`) VALUES ('" + DualTransactionKey + "'," +
                            balayer.TostrEvenNull(Convert.ToString(Session["Branchid"])) + ",'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'," + voucherNo + ",'D', '3','" + choosenDate.ToString("yyyy-MM-dd") + "','" + Narrat +
                            "'," + Convert.ToString(dtVou.Rows[i]["Amount"]) + ",'" + series + "','" + Convert.ToString(dtVou.Rows[i]["ReceievedBy"]) + "',1," + choosenDate.Day + "," + choosenDate.Month + "," +
                            choosenDate.Year + "," + memberId + "," + trans_medium + ", 3, " + chitGrpId + ",1,'" + appReceiptno + "',0,0," + dtVou.Rows[i]["MoneyCollId"] + ",'" + objSession.LoginIp + "');";

                            long insert = trn.insertorupdateTrn(qry1);
                            long insert1 = trn.insertorupdateTrn(qry2);
                            //1131147-> Interest on chit debts headId
                            string qryBank = "insert into transbank(BranchID, TransactionKey, DualTransactionKey, T_Day, T_Month, T_Year, BankHeadID, Head_Id, MemberID, CustomersBankName, DateInCheque, ChequeDDNO, GivenAmount, TotalChequeDDAmount, IsBounced, Trans_Type) " +
                            "values(" + balayer.TostrEvenNull(Convert.ToString(Session["Branchid"])) + "," + insert1 + "," + DualTransactionKey + "," + choosenDate.Day + "," + choosenDate.Month + "," + choosenDate.Year + "," + 11111 /*BankHeadID(Id of name of the bank)*/ + ",1131147," + memberId + ",'Online Transfer','" +
                            choosenDate.ToString("yyyy-MM-dd") + "'," + 123584/*BankReferenceNumber*/+ "," + Convert.ToString(dtVou.Rows[i]["Amount"]) + "," + totalTransBank + ",0,1)";
                ...
                //26/12/2020
                trn.insertorupdateTrn("update mobileappvoucher set IsAccepted=" + true + " where AppReceiptno='" + appReceiptno + "' and Head_Id=" + Head_Id + ";");
        ...
        protected void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            else if (RadView.Checked == true)
            {
                ...
                Appvoucherapproval.Visible = false;
                imgexport.Visible = true;
                string seriesSelected = ddlSeries.SelectedItem.Value;
                approvalview.LoadWithDate(seriesSelected, txtFrom.Text, txtTo.Text);
            }
        }
        ...
        protected void imgexport_Click(object sender, ImageClickEventArgs e)
        {
            ...
            sheet.Range["B6"].Value = "Branch Name";
            ...
            //sheet.Range["C6"].Value = "Approved Date";
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                sheet.Range["B" + rowcount].Value = row.ItemArray[5].ToString();
                //sheet.Range["D" + rowcount].Value = row.ItemArray[8].ToString();
                sheet.Range["C" + rowcount].DateTimeValue = Convert.ToDateTime(row.ItemArray[7]);
                sheet.Range["C" + rowcount].NumberFormat = "dd-MM-yyyy hh:mm:ss";
                sheet.Range["D" + rowcount].Value = row.ItemArray[4].ToString();
                sheet.Range["E" + rowcount].Value = row.ItemArray[0].ToString();
                //06/12/2021
                string narration = row.ItemArray[10].ToString();
                ...
                sheet.Range["G" + rowcount].NumberValue = Convert.ToDouble(row.ItemArray[8]);
                sheet.Range["G" + rowcount].NumberFormat = "#,##0.00";
                amt += Convert.ToDecimal(row.ItemArray[8]);
                sheet.Range["H" + rowcount].NumberValue = Convert.ToDouble(row.ItemArray[9]);
                sheet.Range["H" + rowcount].NumberFormat = "#,##0.00";
                amt1 += Convert.ToDecimal(row.ItemArray[9]);
                sheet.Range["I" + rowcount].NumberValue = Convert.ToDouble(row.ItemArray[11]);
                sheet.Range["I" + rowcount].NumberFormat = "#,##0.00";
                amt2 += Convert.ToDecimal(row.ItemArray[11]);
