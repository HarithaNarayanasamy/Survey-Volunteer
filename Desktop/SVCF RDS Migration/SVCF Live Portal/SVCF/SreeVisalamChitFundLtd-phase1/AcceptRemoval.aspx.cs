...
{
    public partial class AcceptRemoval : System.Web.UI.Page
    {
        ...
        ILog logger = log4net.LogManager.GetLogger(typeof(AcceptRemoval));
        ...
        protected void btnInfo_yes_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (lblHint.Text == "Approve")
                {
                    ...
                    string MemberID = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["NewMemberID"]);
                    string oldMember = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["OldMemberID"]);
                    string GrpMemberId = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["GroupMemberID"]);
                    string date = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["DateOfRemoval"]);
                   // DataTable dtMemberNameandAddress = balayer.GetDataTable("SELECT CustomerName,ResidentialAddress FROM svcf.membermaster where MemberIDNew=" + MemberID);
                    ...
                    // string GroupID = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["ChitGroup"]);
                    string strquery = "update removal_approval set RejectionDate='" + balayer.indiandateToMysqlDate(date) + "',Flag=2,RejectionReason='Another Member Added' where ChitNO=" + ddlChitgroupNo.SelectedItem.Value + " and GroupMemberID=" + GrpMemberId + " and OldMemberID=" + oldMember;
                    string strquery1 = "update removal_approval set RejectionDate=null,Flag=1,RejectionReason=null,SubstitutedDate='" + balayer.indiandateToMysqlDate(date) + "',suggesionifany='"+balayer.MySQLEscapeString(txtSuggession.Text)+"' where ChitNO=" + ddlChitgroupNo.SelectedItem.Value + " and GroupMemberID=" + GrpMemberId + " and OldMemberID=" + oldMember + " and NewMemberID=" + MemberID;
                    //string strUpdateMembertoGroupMaster = "update membertogroupmaster set `MemberName`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(dtMemberNameandAddress.Rows[0]["CustomerName"])) + "',`MemberID`=" + MemberID + ",`MemberAddress`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(dtMemberNameandAddress.Rows[0]["ResidentialAddress"])) + "',`NomineeName`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(dtNomineeNameandAddress.Rows[0]["NomineeName"])) + "',`NomineeAddress`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(dtNomineeNameandAddress.Rows[0]["NomineeAddress"])) + "',`card`=" + balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["card"]) + ",`EstCallNoOfAuction`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["EstimatedDrawNumber"])) + "',`EstSuretyDocument`='" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["EstimatedSurityDetails"])) + "',`M_Id`=" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["M_ID"])) + ",`PoolNo`=null,B_Id=" + balayer.MySQLEscapeString(balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["B_ID"])) + ",headofficesuggession='" + balayer.MySQLEscapeString(txtSuggession.Text) + "' where Head_Id=" + GrpMemberId;
                   ...
                   // long insertUpdateMembertoGroupMaster = trn.insertorupdateTrn(strUpdateMembertoGroupMaster);
                    ...
                    if (lblHint.Text == "Reject")
                    {
                        ...
                        string date = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["DateOfRemoval"]);
                        // string GroupID = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["ChitGroup"]);
                        string strquery = "update removal_approval set RejectionDate='" + balayer.indiandateToMysqlDate(date) + "',Flag=2,RejectionReason='" + balayer.MySQLEscapeString(txtSuggession.Text) + "'  where ChitNO=" + ddlChitgroupNo.SelectedItem.Value + " and NewMemberID=" + MemberID + " and GroupMemberID=" + GrpMemberId + " and OldMemberID=" + oldMember;
                        long insert = trn.insertorupdateTrn(strquery);
                ...
                trn.CommitTrn();
                logger.Info("AcceptRemoval.aspx - btnInfo_yes_Click() - Completed: " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
            }
            catch (Exception error)
            {
                ...
                {
                    logger.Info("AcceptRemoval.aspx - btnInfo_yes_Click() - Error: " + ex.Message + ": " + DateTime.Now + " by: " + Convert.ToString(Session["UserName"]) + "");
                }
                finally
                {
        ...
        {
            string strQueryable = "";
            strQueryable = "SELECT b1.B_Name as BranchName, r1.BranchId,g1.GROUPNO,m1.GrpMemberID,concat(nm1.MemberID,' | ',nm1.CustomerName) as NewMember,nm1.ResidentialAddress as NewMemberAddress,concat(om1.MemberID,' | ',om1.CustomerName) as OldMember,om1.ResidentialAddress as OldMemberAddress,mc1.moneycollname,r1.ChitNO,r1.OldMemberID,r1.GroupMemberID,r1.NewMemberID,DATE_FORMAT(r1.DateOfRemoval, '%d/%m/%Y') as DateOfRemoval,r1.ReasonForRemoval,DATE_FORMAT(r1.`1stNoticeDate`, '%d/%m/%Y') as `1stNoticeDate`,DATE_FORMAT(r1.`2ndNoticeDate`, '%d/%m/%Y') as `2ndNoticeDate`,DATE_FORMAT( r1.SubstitutedDate, '%d/%m/%Y') as SubstitutedDate,DATE_FORMAT( r1.RejectionDate, '%d/%m/%Y') as RejectionDate,r1.commision,r1.CurrentDrawNumber,r1.EstimatedDrawNumber,r1.EstimatedSurityDetails,r1.M_ID,r1.B_ID,r1.Flag,r1.SNo,b2.B_Name as MembersBranchName,r1.monthlyincome,r1.profession,r1.kasar,r1.removalamount FROM svcf.removal_approval as r1 join groupmaster as g1 on (r1.ChitNO=g1.Head_Id) join membertogroupmaster as m1 on (r1.GroupMemberID=m1.Head_Id) join membermaster as nm1 on (r1.NewMemberID=nm1.MemberIDNew)  join membermaster as om1 on (r1.OldMemberID=om1.MemberIDNew) join moneycollector as mc1 on (r1.M_ID=mc1.moneycollid) join branchdetails as b1 on (r1.BranchId=b1.Head_Id) join  branchdetails as b2 on (r1.B_ID=b2.Head_Id) where r1.`ChitNO`=" + ddlChitgroupNo.SelectedItem.Value + " and r1.Flag=0";
        ...
        {
            DataTable dtChitGroupNo = balayer.GetDataTable("SELECT distinct r1.ChitNO,g1.GROUPNO FROM svcf.removal_approval as r1 join groupmaster as g1 on (r1.ChitNO=g1.Head_Id) where r1.Flag=0");
            ddlChitgroupNo.DataValueField = "ChitNO";
        ...
        protected void Audit_Click(object sender, EventArgs e)
        {
            ...
            string memID = balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvRow.RowIndex]["NewMemberID"]);
        ...
        protected void Approve_Click(object sender, ImageClickEventArgs e)
        {
            ...
            GridViewRow gvrow = (GridViewRow)btndetails.NamingContainer;
            Session["Row"] = gvrow;
            string strCustomerName = balayer.GetSingleValue("SELECT CustomerName FROM svcf.membermaster where MemberIDNew=" + balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvrow.RowIndex]["NewMemberID"]));
            string strToken = balayer.GetSingleValue("SELECT GrpMemberID FROM svcf.membertogroupmaster where Head_Id=" + balayer.ToobjectstrEvenNull(GridView1.DataKeys[gvrow.RowIndex]["GroupMemberID"]));
